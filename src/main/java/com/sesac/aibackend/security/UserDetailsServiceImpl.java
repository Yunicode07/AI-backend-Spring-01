...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) {
        User user = userRepository.findByUsername(username) // username 통해 찾음
                .orElseThrow(() -> new UsernameNotFoundException(username)); // 정상적 사용자 아니면 Error

        // Spring Security의 ROLE_ 접두사 컨벤션
        SimpleGrantedAuthority authority = new SimpleGrantedAuthority("ROLE_" + user.getRole()); // 객체에 권한 삽입
